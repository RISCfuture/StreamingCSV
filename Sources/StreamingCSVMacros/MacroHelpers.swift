        ...
        for member in structDecl.memberBlock.members {
            ...
            for attribute in varDecl.attributes {
                guard case .attribute(let attr) = attribute,
                      ...
                      identifierType.name.text == "Fields" else {
                    continue
                }
            ...
            if hasFieldAttribute {
                ...
                if let arrayType = typeAnnotation.as(ArrayTypeSyntax.self) {
                    ...
                    if typeStr.hasPrefix("[") && typeStr.hasSuffix("]") {
                        let elementTypeStr = String(typeStr.dropFirst().dropLast()).trimmingCharacters(in: .whitespaces)
...
enum InitializerGenerator {
    static func generateInitializer(for fields: [FieldInfo], protocolType _: String = "CSVDecodable") -> InitializerDeclSyntax {
